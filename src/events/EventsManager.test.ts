...
describe('EventsManager', () => {
  ...
  describe('registerEvents', () => {
    ...
    it('should register valid events', () => {
      const handler = vi.fn();
      eventsManager.registerEvents([{ type: 'test:event', handler }]);
    ...
    it('should warn and skip invalid event (missing type)', () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('Invalid event registration', {
    ...
    it('should warn and skip invalid event (handler not a function)', () => {
      ...
      eventsManager.registerEvents([{ type: 'test:event', handler: 'not a function' }]);

      expect(loggerSpy).toHaveBeenCalledWith('Invalid event registration', {
        code: 'EVENT_INVALID_REGISTRATION',
        source: 'EventsManager.registerEvents',
        event: { type: 'test:event', handler: 'not a function' },
      });
      loggerSpy.mockRestore();
    });
  ...
  describe('broadcast', () => {
    ...
    it('should handle broadcast errors gracefully', () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('Broadcast error', {
  ...
  describe('WebSocket message handling', () => {
    ...
    it('should warn if no handlers registered for event type', async () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('No handlers registered for event type', {
