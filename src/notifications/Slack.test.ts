...
describe('Slack', () => {
  ...
  describe('sendNotification()', () => {
    ...
    it('should throw error when Slack API returns non-ok response', async () => {
      ...
      await expect(Slack.sendNotification(webhookUrl, 'Test message')).rejects.toThrow(
        'Slack API returned 400: Bad Request'
      );
    });
    ...
    it('should handle 500 server errors', async () => {
      ...
      await expect(Slack.sendNotification(webhookUrl, 'Test message')).rejects.toThrow(
        'Slack API returned 500: Internal Server Error'
      );
    });
  });
  ...
  describe('sendMessage()', () => {
    ...
    it('should throw error when no text, blocks, or attachments provided', async () => {
      await expect(Slack.sendMessage({ webhookUrl })).rejects.toThrow(
        'Slack message must include text, blocks, or attachments'
      );
    });
    ...
    it('should handle API errors with response text', async () => {
      ...
      await expect(Slack.sendMessage({ webhookUrl, text: 'Test' })).rejects.toThrow(
        'Slack API returned 400: Invalid payload'
      );
    });
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(Slack.sendMessage({ webhookUrl, text: 'Test' })).rejects.toThrow(
        'Failed to send Slack message: Network timeout'
      );
    });
