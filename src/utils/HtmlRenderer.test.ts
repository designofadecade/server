...
describe('HtmlRenderer', () => {
  ...
  describe('render()', () => {
    ...
    describe('Each loops', () => {
      ...
      it('should access nested properties in each loop', () => {
        ...
        const vars = {
          users: [{ profile: { age: 25 } }, { profile: { age: 30 } }],
        };
        const result = HtmlRenderer.render(template, vars);
    ...
    describe('Nested structures', () => {
      ...
      it.skip('should handle deeply nested structures (known limitation)', () => {
        ...
        const template =
          '{{#each groups}}{{#if active}}{{#each members}}{{name}} {{/each}}{{/if}}{{/each}}';
        const vars = {
          groups: [
    ...
    describe('Complex templates', () => {
      it('should render a simple list template', () => {
        const template =
          '<div>{{#if users}}<h2>{{title}}</h2><ul>{{#each users}}<li>{{name}}</li>{{/each}}</ul>{{else}}<p>No users</p>{{/if}}</div>';
        const vars = {
  ...
  describe('Security', () => {
    describe('HTML escaping', () => {
      it('should escape markup in interpolated values', () => {
        const result = HtmlRenderer.render('<p>{{name}}</p>', {
      ...
      it('should escape quotes so a value cannot break out of an attribute', () => {
        const result = HtmlRenderer.render('<a title="{{t}}">x</a>', {
          t: '" onmouseover=alert(1) z="',
        });

        // The payload survives as inert text inside the title value; what
        // matters is that the quote is escaped, so it never closes the
        // attribute and no new attribute is created.
        expect(result).toBe('<a title="&quot; onmouseover=alert(1) z=&quot;">x</a>');
      });

      it('should escape ampersands and single quotes', () => {
        const result = HtmlRenderer.render('{{v}}', { v: `Tom & "Jerry" it's` });

        expect(result).toBe('Tom &amp; &quot;Jerry&quot; it&#39;s');
      });

      it('should emit raw HTML through the triple-brace form', () => {
        const result = HtmlRenderer.render('<div>{{{html}}}</div>', {
          html: '<b>bold</b>',
        });
      ...
      it('should escape values inside an each block', () => {
        const result = HtmlRenderer.render('{{#each items}}<li>{{name}}</li>{{/each}}', {
    ...
    describe('template injection through data', () => {
      // Values substituted inside an {{#each}} used to be written straight
      // back into the working string, so the next pass of the loop read them
      // as template syntax.
      it('should not resolve a variable reference supplied as data', () => {
        const result = HtmlRenderer.render('{{#each items}}{{name}}{{/each}}', {
          secret: 'S3CRET',
          items: [{ name: '{{secret}}' }],
        });
      ...
      it('should not execute an if block supplied as data', () => {
        const result = HtmlRenderer.render('{{#each items}}{{name}}{{/each}}', {
          admin: true,
          items: [{ name: '{{#if admin}}PWNED{{/if}}' }],
        });

        // Evaluating the block would collapse it to just 'PWNED'. Emitting
        // it verbatim shows it was treated as data, not template syntax.
        expect(result).toBe('{{#if admin}}PWNED{{/if}}');
      });

      it('should not execute an each block supplied as data', () => {
        const result = HtmlRenderer.render('{{#each items}}{{name}}{{/each}}', {
          others: ['a', 'b'],
          items: [{ name: '{{#each others}}X{{/each}}' }],
        });

        expect(result).not.toBe('XX');
      });

      it('should not leak a sibling value through a raw interpolation', () => {
        const result = HtmlRenderer.render('{{#each items}}{{{name}}}{{/each}}', {
          secret: 'S3CRET',
          items: [{ name: '{{secret}}' }],
        });
      ...
      it('should ignore a forged slot sentinel in the template', () => {
        const result = HtmlRenderer.render('\u00000\u0000{{v}}', { v: 'ok' });
  ...
  describe('renderFromFile()', () => {
    ...
    it('should handle file read errors', async () => {
      ...
      await expect(HtmlRenderer.renderFromFile('/invalid/path.html', {})).rejects.toThrow(
        'File not found'
      );
    });
