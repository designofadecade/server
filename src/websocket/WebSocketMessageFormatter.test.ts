...
describe('WebSocketMessageFormatter', () => {
  ...
  describe('parse', () => {
    ...
    it('should return null for invalid JSON', () => {
      ...
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          ...
          return (
            parsed.level === 'ERROR' &&
            parsed.message === 'Failed to parse incoming WebSocket message as JSON'
          );
        } catch {
          return false;
        }
    ...
    it('should return null for empty string', () => {
      ...
      expect(result).toBeNull();
      // Check that console.log was called with JSON containing the warning
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          const parsed = JSON.parse(call[0]);
          return parsed.level === 'WARN' && parsed.message === 'Received invalid message data';
        } catch {
          return false;
        }
      });
    ...
    it('should return null for message without type', () => {
      ...
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          const parsed = JSON.parse(call[0]);
          return (
            parsed.level === 'ERROR' && parsed.message === 'Parsed message lacks required fields'
          );
        } catch {
          return false;
        }
