    ...
    it('should log success message when server starts listening', () => {
      ...
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          ...
          return parsed.level === 'INFO' && parsed.message === 'WebSocket Server listening';
        } catch {
          return false;
        }
      });
  ...
  describe('close', () => {
    it('should close the WebSocket server successfully', async () => {
      ...
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          const parsed = JSON.parse(call[0]);
          return parsed.level === 'INFO' && parsed.message === 'WebSocket Server closed';
        } catch {
          return false;
        }
      });
    ...
    it('should handle close errors', async () => {
      ...
      const logCall = consoleSpy.mock.calls.find((call) => {
        try {
          const parsed = JSON.parse(call[0]);
          return parsed.level === 'ERROR' && parsed.message === 'Error closing WebSocket Server';
        } catch {
          return false;
        }
      });
